# %% ===== 101 Formulaic Alphas -> 공통 IR =====
"""
101 Formulaic Alphas (Kakushadze 2015, WorldQuant) 프론트엔드.

- 입력: 논문 Appendix A.1 의 수식 (닫힌 DSL — 문법은 A.2 에 명세)
- 출력: qant_ir 공통 스키마의 IR 트리 -> JPX Stage A 결과와 동일 포맷
- 축은 추론하지 않는다: 논문 명세가 연산자별 축을 선언하므로 테이블 매핑.

실행:  python qant_101_alphas.py
결과:  ./qant_out/qant_101_ir.json, qant_101_census.csv
       (qant_out/qant_stage_a_ir.json 이 있으면 JPX와 병합 센서스도 출력)
"""
import re, json
...
def tokenize(s):
    ...
    while i < len(s):
        m = TOK.match(s, i)
        if not m:
            raise SyntaxError(f"tokenize fail at {i}: {s[i:i+20]!r}")
        i = m.end()
        if m.lastgroup == "ws":
            continue
...
# ---------------- 연산자 -> IR 매핑 (논문 A.2 명세) ----------------
FIELDS = {"open", "close", "high", "low", "volume", "vwap", "returns", "cap"}
ADV_RE = re.compile(r"^adv(\d+)$")

TS_1 = {  # (x, d) -> 마지막 인자가 윈도우
    "ts_rank": "ts_rank", "ts_min": "ts_min", "ts_max": "ts_max",
    "ts_argmax": "ts_argmax", "ts_argmin": "ts_argmin",
    "sum": "ts_sum", "product": "ts_product", "stddev": "ts_std",
    "delay": "ts_delay", "delta": "ts_delta", "decay_linear": "ts_decay_linear",
}
TS_2 = {"correlation": "ts_corr", "covariance": "ts_cov"}   # (x, y, d)
ELEM = {"abs": "abs", "log": "log", "sign": "sign", "signedpower": "signedpower"}
BIN = {"+": "add", "-": "sub", "*": "mul", "/": "div", "^": "pow",
       "<": "lt", ">": "gt", "<=": "le", ">=": "ge", "==": "eq", "!=": "ne",
       "||": "or_", "&&": "and_"}

def mk(op, args, params=None, axis=None, by=None):
    n = {"kind": "op", "op": op, "axis": axis, "args": args, "params": params or {}}
    if by:
        n["by"] = by
    return n
...
def is_num(ir):
    return isinstance(ir, dict) and ir.get("kind") == "const" and isinstance(ir.get("value"), (int, float))
...
class P:
    ...
    def eat(self, v):
        k, s = self.next()
        if s != v: raise SyntaxError(f"expected {v!r}, got {s!r}")

    def expr(self):          # 삼항 (최저 우선순위, 우결합)
        cond = self.or_()
        if self.peek()[1] == "?":
            self.next(); a = self.expr(); self.eat(":"); b = self.expr()
            return mk("where", [cond, a, b])
        return cond

    def or_(self):
        n = self.and_()
        while self.peek()[1] == "||":
            self.next(); n = mk("or_", [n, self.and_()])
        return n

    def and_(self):
        n = self.cmp()
        while self.peek()[1] == "&&":
            self.next(); n = mk("and_", [n, self.cmp()])
        return n

    def cmp(self):
        n = self.add()
        while self.peek()[1] in ("<", ">", "<=", ">=", "==", "!="):
            op = self.next()[1]; n = mk(BIN[op], [n, self.add()])
        return n

    def add(self):
        n = self.mul()
        while self.peek()[1] in ("+", "-"):
            op = self.next()[1]; n = mk(BIN[op], [n, self.mul()])
        return n

    def mul(self):
        n = self.unary()
        while self.peek()[1] in ("*", "/"):
            op = self.next()[1]; n = mk(BIN[op], [n, self.unary()])
        return n
    ...
    def power(self):                            # ^ 는 거듭제곱, 우결합
        n = self.atom()
        if self.peek()[1] == "^":
            self.next(); return mk("pow", [n, self.unary()])
        return n
    ...
    def atom(self):
        ...
        if kind == "num":
            return const(float(s) if ("." in s or "e" in s or "E" in s) else int(s))
        if kind == "id":
            name = s.lower()
            # IndClass.sector 류
            if name == "indclass":
                self.eat("."); lvl = self.next()[1].lower()
                return {"kind": "const", "value": {"indclass": lvl}}
            if self.peek()[1] == "(":
                self.next()
                args = []
                if self.peek()[1] != ")":
            ...
            # 필드
            if name in FIELDS or ADV_RE.match(name):
                return {"kind": "field", "name": name}
            raise SyntaxError(f"unknown identifier {s!r}")
        raise SyntaxError(f"unexpected token {s!r}")

    # ---- 호출을 논문 명세에 따라 IR로 ----
    def call(self, name, args):
        if name in ELEM:
            return mk(ELEM[name], args)
        if name == "rank":
            return mk("cs_rank", args, axis="CS")
        if name == "scale":
            params = {}
            if len(args) == 2 and is_num(args[1]):
                params["a"] = args.pop(1)["value"]
            return mk("cs_scale", args, params, axis="CS")
        if name == "indneutralize":
            by = None
            if len(args) == 2 and isinstance(args[1], dict) and \
               isinstance(args[1].get("value"), dict) and "indclass" in args[1]["value"]:
                by = [args.pop(1)["value"]["indclass"]]
            return mk("grp_demean", args, axis="GRP", by=by)
        if name in TS_1:
            params = {}
            if args and is_num(args[-1]):
                params["window"] = args.pop(-1)["value"]
            return mk(TS_1[name], args, params, axis="TS")
        if name in TS_2:
            params = {}
            if args and is_num(args[-1]):
                params["window"] = args.pop(-1)["value"]
            return mk(TS_2[name], args, params, axis="TS")
        if name in ("min", "max"):
            # 논문: min(x, d)=ts_min. 단 두 인자가 모두 시리즈면 원소별 min/max.
            if len(args) == 2 and is_num(args[1]):
                return mk("ts_" + name, [args[0]], {"window": args[1]["value"]}, axis="TS")
            return mk(name, args)
        raise SyntaxError(f"unknown function {name!r}")


def parse_alpha(formula):
    return P(tokenize(formula)).expr()


# ---------------- 101개 수식 (논문 Appendix A.1 전문) ----------------
# 출처: Kakushadze, "101 Formulaic Alphas" (2015). 수식은 WorldQuant LLC 소유,
# 논문 게재 허가 하 공개. 내부 연구용 파싱/통계 목적으로만 사용.
ALPHAS = {
...
if __name__ == "__main__":
    rows, fails = [], []
    for aid, formula in ALPHAS.items():
        try:
            raw = parse_alpha(formula)
            can = normalize(raw)
            rows.append({
                "corpus": "101alphas",
                "alpha_id": f"alpha_{aid:03d}",
                "formula": formula,
                # 이중 저장: raw = 원본 보존/디버깅, canonical = 검색/중복/RAG
                "ir_raw": raw, "sexpr_raw": sexpr(raw), "hash_raw": shash(raw),
                "ir_canonical": can, "sexpr_canonical": sexpr(can),
                "hash_canonical": shash(can),
                # 레거시 키 = canonical (소비 코드 기본값)
                "ir": can, "sexpr": sexpr(can), "hash": shash(can),
                "kind": classify(can),
                "depth": tree_depth(can),
                "ops": ops_in(can),
                "fields": sorted(fields_in(can)),
            })
        except Exception as e:
            fails.append((aid, str(e)[:100]))

    print(f"파싱: {len(rows)}/101  실패: {len(fails)}")
    for aid, msg in fails:
        print(f"  [FAIL] alpha#{aid}: {msg}")

    df = pd.DataFrame([{k: v for k, v in r.items() if k != "ir"} for r in rows])
    print("\n분류:", dict(df["kind"].value_counts()))
    print("유니크 해시:", df["hash"].nunique())

    # 샘플 확인
    for aid in (101, 3, 9, 58):
        r = [x for x in rows if x["alpha_id"] == f"alpha_{aid:03d}"]
        if r:
            print(f"\n#\u007b{aid}\u007d {r[0]['sexpr'][:120]}")
    ...
    # JPX 결과가 있으면 병합 센서스
    jpx_path = OUTDIR / "qant_stage_a_ir.json"
    if jpx_path.exists():
        jpx = json.loads(jpx_path.read_text(encoding="utf-8"))
        cen_j = Counter()
        n_factors = 0
        for f in jpx:
            ir = f.get("ir_norm") or f.get("ir")   # 정규화된 IR 우선
            if isinstance(ir, dict) and classify(ir) == "factor":
                n_factors += 1
                for op, ax in ops_in(normalize(ir)):
                    cen_j[op] += 1
        allops = sorted(set(cen) | set(cen_j))
        merged = pd.DataFrame([{
            "op": o, "n_101": cen.get(o, 0), "n_jpx": cen_j.get(o, 0),
            "total": cen.get(o, 0) + cen_j.get(o, 0),
            "in_both": (cen.get(o, 0) > 0) and (cen_j.get(o, 0) > 0),
        } for o in allops]).sort_values("total", ascending=False)
        print(f"\n=== 병합 센서스 (101 alphas + JPX 팩터 {n_factors}개) ===")
        print(merged.to_string(index=False))
        merged.to_csv(OUTDIR / "qant_merged_census.csv", index=False)

    # 내보내기 (JPX와 동일 스키마)
    from qant_ir import NORMALIZER_VERSION
    meta = {"source": "101 Formulaic Alphas (Kakushadze 2015)",
            "normalizer_version": NORMALIZER_VERSION,
            "license_note": "Formulas proprietary to WorldQuant LLC, published with permission; internal research use only",
            "n_parsed": len(rows)}
    with open(OUTDIR / "qant_101_ir.json", "w", encoding="utf-8") as fp:
        json.dump({"meta": meta, "alphas": rows}, fp, ensure_ascii=False, indent=1, default=str)
    census.to_csv(OUTDIR / "qant_101_census.csv", index=False)
    print("\n저장: qant_101_ir.json / qant_101_census.csv" +
          (" / qant_merged_census.csv" if jpx_path.exists() else ""))
